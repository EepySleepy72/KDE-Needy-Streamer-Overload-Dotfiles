- **Distro** - [CachyOS](https://cachyos.org/)
- **Wallpaper** - Dynamic Wallpaper using Needy Streamer Assets. Day and Night wallpapers found above.
- **Plasma Style** - [Reactionary Plus](https://store.kde.org/p/2138468)
- **Window Decorations / Color Theme / System Sounds**- [Plasma Overdose](https://store.kde.org/u/notify-ctrl)
- **App Style** - MS Windows 9X (Built-In)
- **Icons** - [Windows 2000 SVG](https://store.kde.org/p/1955362) + Needy Streamer Assets
- **Fastfetch** - Config Found Above
- **Font** - [zpix](https://github.com/SolidZORO/zpix-pixel-font) (General) / Departure Nerd Font (Konsole)
- **Taskbar Widgets** - App Launcher / Icon&Text Taskbar / Media Frame / Digital Clock (Built in)

Image and config files used can be found above in the appropriate folders.
Additional Needy Streamer Overload assets can be found in this [Google Drive](https://drive.google.com/drive/folders/1Hdso9LGL6FtbAoC8nPF_S8trc0weMqsv) by u/My_man___

<img width="2557" height="1440" alt="unixpornNSO" src="https://github.com/user-attachments/assets/e7abf695-e5bf-46e2-92ea-58a3f84ab460" />
